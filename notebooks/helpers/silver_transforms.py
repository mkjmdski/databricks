...
def clean_address_silver(address_bronze: DataFrame) -> DataFrame:
    ...
    return address_bronze.withColumn(
        "full_address",
        when(col("address2").isNotNull(), concat_ws(", ", col("address"), col("address2"))).otherwise(col("address")),
    )
...
def build_address_with_location(
    address_silver: DataFrame, city_bronze: DataFrame, country_bronze: DataFrame
) -> DataFrame:
    """
...
def transform_staff_to_gold(staff_bronze: DataFrame, address_with_location: DataFrame) -> DataFrame:
    ...
    return (
        staff_with_manager.join(address_with_location.alias("addr"), "address_id", "left")
        .select(
...
def transform_store_to_gold(
    store_bronze: DataFrame, address_with_location: DataFrame, staff_bronze: DataFrame
) -> DataFrame:
    """
...
def transform_car_to_gold(
    inventory_silver: DataFrame,
    car_bronze: DataFrame,
    inventory_equipment_bronze: DataFrame,
    equipment_bronze: DataFrame,
) -> DataFrame:
    """
    ...
    equipment_agg = (
        ...
        .agg(concat_ws(", ", array_sort(collect_set(col("eq.name")))).alias("equipment_list"))
    )
...
def transform_customer_full_pipeline(
    customer_bronze: DataFrame, address_bronze: DataFrame, city_bronze: DataFrame, country_bronze: DataFrame
) -> DataFrame:
    """
...
def transform_staff_full_pipeline(
...
def transform_store_full_pipeline(
    ...
    address_bronze: DataFrame,
    city_bronze: DataFrame,
    country_bronze: DataFrame,
) -> DataFrame:
    """
...
def transform_car_full_pipeline(
    ...
    return transform_car_to_gold(inventory_silver, car_bronze, inventory_equipment_bronze, equipment_bronze)
...
def generate_date_dimension(
    ...
    dates_df = spark.range(1).select(explode(sequence(lit(start_date), lit(end_date))).alias("date"))
...
def generate_role_playing_date_dimensions(
    base_dim_date: DataFrame, role_names: list[tuple[str, str]]
...
def build_staff_hierarchy_bridge(staff_bronze: DataFrame, max_depth: int = 10) -> DataFrame:
    ...
    for i in range(1, max_depth + 1):
        hierarchy = hierarchy.withColumn(
            "last_manager_key", when(size(col("manager_keys")) >= i, element_at(col("manager_keys"), i))
        )
        ...
        hierarchy = (
            ...
            .withColumn(
                "manager_keys",
                when(
                    col("new_manager_key").isNotNull(), concat(col("manager_keys"), array(col("new_manager_key")))
                ).otherwise(col("manager_keys")),
            )
            .withColumn("path", col("manager_keys"))
...
def transform_equipment_dimension(equipment_bronze: DataFrame) -> DataFrame:
    ...
    """
    return equipment_bronze.withColumn("equipment_key", xxhash64(col("equipment_id"))).select(
        "equipment_key", "equipment_id", "name", "type", "version"
    )


def build_equipment_bridges(inventory_equipment_bronze: DataFrame) -> tuple[DataFrame, DataFrame]:
    """
    ...
    equipment_groups = (
        inventory_equipment_bronze.groupBy("inventory_id")
    ...
    bridge_equipment_group_equipment = (
        equipment_groups.withColumn("equipment_id", explode(col("equipments_array")))
    ...
    logger.info(
        f"Equipment bridges built: {bridge_equipment_group_equipment.count():,} group-equipment, {bridge_car_equipment.count():,} car-equipment"
    )
...
def transform_rental_fact(
    rental_bronze: DataFrame, staff_bronze: DataFrame, inventory_bronze: DataFrame, payment_bronze: DataFrame
) -> DataFrame:
    """
    ...
    rental_with_staff = (
        rental_bronze.alias("rental")
        .join(
            staff_bronze.select(col("staff_id"), col("store_id")).alias("staff"),
    ...
    fact_rental = (
        ...
        .withColumn(
            "return_date_key", when(col("return_date").isNotNull(), xxhash64(col("return_date"))).otherwise(None)
        )
        .withColumn(
            "payment_date_key", when(col("payment_date").isNotNull(), xxhash64(col("payment_date"))).otherwise(None)
        )
        .withColumn("payment_deadline_date_key", xxhash64(col("payment_deadline")))
        # Add calculated business metrics
        .withColumn("rental_amount", col("rental_rate") * datediff(col("return_date"), col("rental_date")))
